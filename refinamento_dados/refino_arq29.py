from util_etl import leitura_inicial_dados, juncao, calcula_tempo, show_df
import pandas as pd
import time
import uuid

def main_refino_arq29(caminho_arq_21: str, caminho_arq_1: str, print_flag: bool = False) -> None:
    ...
    dataf_inicial_arq29 = leitura_inicial_dados(caminho_arq_21, dic_de_tipagem={"NU_ANO": "int"})
    dataf_filtrado_arq29 = dataf_inicial_arq29.dropna(how="all")

    dataf_filtrado_arq29 = dataf_filtrado_arq29[
        dataf_filtrado_arq29["CO_CURSO"].isin(dataf_final_arq1)
    ].copy()
    ids_unicos = [str(uuid.uuid4()) for _ in range(len(dataf_filtrado_arq29))]
    dataf_filtrado_arq29.insert(0, "id", ids_unicos)

    pasta_destino = "G:/Meu Drive/01_Faculdade/03_Projeto tecnologico/Biblioteca de dados Enade/Modelo_teste/associacao/arq29_refinado-2.csv"
    dataf_filtrado_arq29.to_csv(pasta_destino, index=False)

    show_df(dataf_filtrado_arq29) if print_flag else None

if __name__ == "__main__":
    caminho_arq_21: str = "G:/Meu Drive/01_Faculdade/03_Projeto tecnologico/Biblioteca de dados Enade/microdados_Enade_2021_LGPD/2.DADOS/microdados2021_arq29.txt"
    ...
    main_refino_arq29(caminho_arq_21, caminho_arq_1)